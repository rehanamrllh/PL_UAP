...
/**
 * Simple date picker field for Swing that exposes LocalDate API.
 *
 * API:
 * - {@link #getDate()} returns selected/typed date as LocalDate (null if empty)
 * - {@link #setDate(LocalDate)} sets date
 * - fires property change "date" on changes
 */
public class LocalDatePickerField extends JPanel {
    ...
    /**
     * Returns the selected date as LocalDate.
     *
     * @return LocalDate, or null if field is empty
     * @throws DateTimeParseException if the text is not a valid YYYY-MM-DD date
     */
            ...
            for (int day = 1; day <= daysInMonth; day++) {
                ...
                if (buttonDate.equals(today)) {
                    ...
                    try {
                        ...
                        dayButton.setForeground(new Color(41, 128, 185));
                    }
                } else {
