...
public class ToDoApp extends JFrame {
    ...
    class TaskListPanel extends JPanel {
        ...
        public TaskListPanel() {
            ...
            // Note: Edit button logic requires populating AddTaskPanel, simplified here for
            // brevity
            btnEdit.addActionListener(e -> {
                ...
                if (taskToEdit == null) {
                    JOptionPane.showMessageDialog(this, "Task not found for edit: " + id);
                    return;
                }

                addTaskPanel.startEdit(taskToEdit);
                cardLayout.show(mainContentPanel, "ADD");
            });
        }
        ...
        private void filterTable() {
            ...
            if (text.length() > 0)
                filters.add(RowFilter.regexFilter("(?i)" + text)); // Case insensitive
            if (!status.equals("ALL"))
                filters.add(RowFilter.regexFilter(status, 4)); // Col 4 is Status
    ...
    class AddTaskPanel extends JPanel {
        ...
        private final LocalDatePickerField dueDatePicker;
        ...
        public AddTaskPanel() {
            ...
            addFormRow(formPanel, gbc, 0, "Task Title:*", txtTitle);
            ...
            btnPanel.setBackground(BG_COLOR);
            btnSave = createStyledButton("Save Task", BTN_GREEN);
        ...
        private void saveTask() {
            try {
                ...
                String id = isEdit ? editingId : UUID.randomUUID().toString().substring(0, 4); // Short ID
                ...
                if (isEdit) {
                    dataManager.updateTask(id, taskToSave);
                    JOptionPane.showMessageDialog(this, "Task Updated Successfully!");
                } else {
                    dataManager.addTask(taskToSave);
                    JOptionPane.showMessageDialog(this, "Task Saved Successfully!");
                }

                clearForm();
                refreshAll();
                cardLayout.show(mainContentPanel, "LIST");
            ...
            for (Task t : dataManager.getTasks()) {
                if (t.getStatus().equals("Completed")) {
                    model.addRow(new Object[] { t.getId(), t.getTitle(), t.getDescription(), t.getPriority(),
                            t.getDueDate() }); // Using due date as completion for demo
                }
            }
        }
    }
